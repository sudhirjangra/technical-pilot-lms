...
export class TestsService {
  ...
  async submitAttempt(
    ...
    const testRel = attempt.tests as {
      id?: string;
      title?: string;
    ...
    const courseObj = testRel?.lessons?.chapters?.courses;
    ...
    await this.mongoService.saveAttempt({
      ...
      lesson_id: testRel?.lesson_id ?? null,
      assessment_id: attempt.test_id,
      assessment_title: testRel?.title ?? null,
    ...
    // Update Supabase reference attempt
  ...
  async getMyAttempts(studentId: string) {
    ...
    return (attempts ?? []).map((a: any) => {
      ...
      return {
        id: a.id,
        type: 'test' as const,
        testTitle: test?.title ?? mongo?.assessment_title ?? 'Test',
  ...
  async getAttemptDetail(attemptId: string, studentId?: string) {
    ...
    if (mongoDoc) {
      ...
      return {
        ...
        test_id: mongoDoc.assessment_id,
    ...
    if (attemptErr || !attempt) {
      // Fallback check in assignment_attempts in case an attempt ID was cross-referenced
      const { data: assignmentAttempt } = await this.supabase
      ...
      if (assignmentAttempt) {
        if (studentId && assignmentAttempt.student_id !== studentId) {
          throw new ForbiddenException('Access denied');
        }
        return this.getAssignmentAttemptDetailDirect(assignmentAttempt);
      }
    ...
    if (attempt.completed_at && this.mongoService.isConnected()) {
      this.attemptMigrationService
        .migrateSingleTestAttempt(attempt.id)
        .catch((err) => {
          this.logger.warn(
            `Failed lazy migration for test attempt ${attempt.id}: ${err instanceof Error ? err.message : String(err)}`,
          );
        });
    }
  ...
  private async getAssignmentAttemptDetailDirect(attempt: any) {
